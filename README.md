...
`@file-viewer/react-full` 默认把 PDF、DOCX、PPT、PPTX、Excel、CAD、Typst、Draw.io、SQLite 和 Archive 资产指向部署基址下的 `file-viewer/`（根部署即 `/file-viewer/`）。运行随包安装的同版本 `file-viewer-copy-assets` 并发布其产物后，不需要逐项手写资源 URL。
